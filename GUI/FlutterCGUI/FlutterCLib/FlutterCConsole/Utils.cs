...
namespace FlutterCConsole
{
    public static class Tools
    {
                                            ...
                                            char Padding, object Value)
        {
            ...
            {
                w = Convert.ToString(UnboxToLong((long)(decimal)Value, true), 8);
    ...
    public class Utils
    {
        ...
        public static char NAME_DELIMITER = '$';

        public static char[] markers = { 'd', 'p', 'x', 'e', 'f', 'c', 's' };

        public static bool isBorderToken(string token)
        {
            return token == ";" || token == "}";
        }

        public static void printMap<T, V>(Dictionary<T, V> d)
        {
            foreach (var fuck in d)
            {
                Console.WriteLine(fuck.Key.ToString() + " " + fuck.Value.ToString());
            }
        }

        public static bool checkVarName(string varname)
        {
            if (varname.Length == 0)
            {
                return false;
        ...
        public static string getStringByAddress(int address)
        {
            if(address == 0){
                return "NULL";
            }
            ...
            string formatString = "";
            int sizefs = Memory.getInstance().cellMemory.sizes[address];

            for (int i = 0; ; i++)
            {
                if (Memory.getInstance().cellMemory.getValue(address + i).value == 0)
                {
                    break;
                }
                formatString += (char)Memory.getInstance().cellMemory.getValue(address + i).value;
            }
            return formatString;
        }

        public static object convert(char marker, decimal value)
        {
            if (marker == 'd' || marker == 'p' || marker == 'x')
            {
                return (long)value;
            }
            ...
            if (marker == 's')
            {
                return getStringByAddress((int)value);
            }

            return null;
        }

        public static void fill(char marker, decimal value, string token)
        {
            if (marker == 'd')
            {
                int v = int.Parse(token);
                Memory.instance.cellMemory.getValue((int)value).value = v;
            }

            if (marker == 'x' || marker == 'p')
            {
                int v = int.Parse(token, System.Globalization.NumberStyles.HexNumber);
                Memory.instance.cellMemory.getValue((int)value).value = v;
            }

            if (marker == 'f' || marker == 'e')
            {
                double v = double.Parse(token);
                Memory.instance.cellMemory.getValue((int)value).value = (decimal)v;
            }
            ...
            if (marker == 's')
            {
                ...
                {
                    Memory.instance.cellMemory.getValue((int)value + i).value = (decimal)(token[i]);
                }
                Memory.instance.cellMemory.getValue((int)value + token.Length).value = 0;
            }
        }

        public static string output(string format, decimal[] values)
        {
            ...
            for (int i = 0; i < format.Length; i++)
            {
                ...
                if (kurok)
                {
                    if (markers.Contains(format[i]))
                    {
                        converted[pointer] = convert(format[i], values[pointer]);
                        pointer++;
                        kurok = false;
                    }
                }
            ...
            string r = Tools.sprintf(format, converted);
            r = r.Replace("\\n", "\n");
            return r;
        }

        public static void scanf(string format, string[] tokens, decimal[] vs)
        {
            bool kurok = false;
            int pointer = 0;
            for (int i = 0; i < format.Length; i++)
            {
                ...
                if (kurok)
                {
                    if (markers.Contains(format[i]))
                    {
                        fill(format[i], vs[pointer], tokens[pointer]);
                        pointer++;
                        kurok = false;
                    }
                }
        ...
        public static bool checkBalance(string s1, string s2, List<string> tokens)
        {
            int count = 0;
        ...
        public static bool isPrefix(string shorStr, string longStr)
        {
            if (longStr.IndexOf(shorStr) == 0)
            {
                return true;
            }
            else
        ...
        public static int indexOf(string token, List<string> forSearch)
        {
            for (int i = 0; i < forSearch.Count; i++)
            {
                if (forSearch[i] == token)
                {
                    return i;
                }
            }
        ...
        public static string[] keywords = { "int", "long", "double", "float", "char", "while", "for" };

        public static string eliminateComments(string s)
        {
            string res = "";
            bool fuckedComment1 = false;
            bool fuckedComment2 = false;
            bool isStr = false;

            for (int i = 0; i < s.Length; i++)
            {
                if (!fuckedComment1 && !fuckedComment2)
                {
                    if (s[i] == '\"')
                    {
                        isStr = !isStr;
                ...
                if (!isStr)
                {
                    ...
                    if (i <= s.Length - 2 && s[i] == '/' && s[i + 1] == '*' && !fuckedComment1)
                    {
                        fuckedComment2 = true;
                    }
                    ...
                    if (s[i] == '\n')
                    {
                        fuckedComment1 = false;
                    }

                    if (i > 0 && s[i] == '/' && s[i - 1] == '*')
                    {
                        fuckedComment2 = false;
                    }
                }
                else
                {
        ...
        public static List<string> merge(List<List<string>> tokens, int begin, int end)
        {
            List<string> res = new List<string>();
            for (int i = begin; i <= end; i++)
            {
                res.AddRange(tokens[i]);
            }
            return res;
        }
        ...
        /*
        public static string merge(List<string> list, string del)
        {
            string res = "";
            for (int i = 0; i < list.Count; i++)
            {
        ...
        {
            return name.Substring(name.LastIndexOf(NAME_DELIMITER) + 1);
        }

        public static string getNodeName(string name)
        {
            return name.Substring(0, name.LastIndexOf(NAME_DELIMITER));
        }

        public static string merge(List<string> l, string delimiter)
        {
            string res = "";
